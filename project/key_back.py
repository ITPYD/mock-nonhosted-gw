...
# Resources:
# https://github.com/UglyBaliCode/RSA-Encryption-with-Python/tree/master/sign%20and%20verify
# https://www.youtube.com/watch?v=LCZ7PcYGxew
#
...
from project.models import MacKey

class Key:
    #public_key_path = app.config['KEY_PATH'] + "/public.key"
    #private_key_path = app.config['KEY_PATH'] + "/private.key"
    ...
    def GenKeys(self, save=True):

        key = RSA.generate(2048)
        
        # --------- replace db key
        #private_key = key.export_key()
        #file_out = open(self.private_key_path, "wb")
        #file_out.write(private_key)
        #file_out.close()

        #public_key = key.publickey().export_key()
        #file_out = open(self.public_key_path, "wb")
        #file_out.write(public_key)
        #file_out.close()
        # ---------- end

        mackey = MacKey()
        mackey.mid = self.mMid
        mackey.tid = self.mTid
        #mackey.private 
        #mackey.public 
        self.pubkey = mackey.public = key.publickey().export_key()
        self.privkey = mackey.private =  key.export_key()

        print("Function:GenKey start")
        print(mackey)

        if save == True:
            db.session.add(mackey)
            print("Function:GenKey - commit db")
            db.session.commit()

        print("Function:GenKey end")
    ...
    def GetPublicKey(self):
        ...
        # --- replace db
        #pubkey = open(self.public_key_path).read()
        # --- end
        print(f"MID:{self.mMid} tid:{self.mTid}")
        mk = MacKey.query.filter_by(mid=self.mMid, tid=self.mTid).first()
        pubkey = mk.public
        #print(pubkey)
        #print("....")
    ...
    def Sign(self, data, private="", public=""):
        #print("[" + str(self.mMid) + "][" + self.mTid + "]===Sign[" + data + "]====")

        # --- replace db
        #key = RSA.import_key(open(self.private_key_path).read())
        # --- end
        if private == "" and public == "":
            mk = MacKey.query.filter_by(mid=self.mMid, tid=self.mTid).first()
            key = RSA.import_key(mk.private)  
        else:
        ...
        print("-----------")
        if public == "":
            self.IsValidSign(mk.public, data, urlSafeEncodedStr)
        else:
            self.IsValidSign(public, data, urlSafeEncodedStr)
        
        return urlSafeEncodedStr

    def IsValidSign(self, pubkey, data, signature):
        print(f"===IsValidSign====[{pubkey}]")
        ...
        #key = RSA.import_key(open(self.public_key_path).read())
